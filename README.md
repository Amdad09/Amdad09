<div align="center">
  <a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=27&duration=4000&pause=1000&color=61DAFB&center=true&vCenter=true&width=600&lines=Hi+👋,+I'm+Amdadul+Haque;Frontend+Developer;React+|+JavaScript+|+Tailwind+CSS;Building+Modern+Web+Applications" alt="Typing SVG" /></a>
</div>

---

## 🎯 Welcome to My GitHub Profile!

<div align="center">
  <p>
    <strong>Passionate Frontend Developer | React Enthusiast | Web Application Builder</strong>
  </p>
  <p>
    I create clean, responsive, and scalable web applications using modern JavaScript frameworks.
    <br/>
    Always learning, always building, always improving.
  </p>
</div>

<div align="center">

### 📍 Quick Navigation

| 👤 **About** | 🛠️ **Tech Stack** | 💼 **Projects** | 📊 **Stats** | 📞 **Contact** |
|:---:|:---:|:---:|:---:|:---:|
| [View](#-about-me) | [View](#-tech-stack) | [View](#-featured-projects) | [View](#-github-statistics) | [View](#-get-in-touch) |

</div>

---

## 🚀 About Me

I'm a dedicated **Frontend Developer** with a passion for creating clean, responsive, and user-centric web applications. I specialize in modern JavaScript frameworks and focus on writing maintainable, scalable code that makes a real impact.

- 💻 **Expertise:** React.js, JavaScript (ES6+), Tailwind CSS, Firebase
- 🎨 **Focus Areas:** Component-driven development, UI/UX best practices, performance optimization
- ⚡ **Strengths:** State management, REST API integration, responsive design patterns
- 🔄 **Currently Mastering:** TypeScript, Advanced React patterns, Full Stack Development
- 🎯 **Vision:** Become a Full Stack Developer & contribute to open-source projects

---

## 🛠️ Tech Stack

### 💻 Frontend Development
<div align="center">

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB&labelColor=20232A)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=323330&labelColor=F7DF1E)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34C26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

</div>

### ⚙️ State Management & Libraries
<div align="center">

![Context API](https://img.shields.io/badge/Context%20API-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Redux](https://img.shields.io/badge/Redux-764ABC?style=for-the-badge&logo=redux&logoColor=white)

</div>

### 🔧 Tools & Platforms
<div align="center">

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-0078D4?style=for-the-badge&logo=visual-studio-code&logoColor=white)

</div>

### 🔥 Backend & Services
<div align="center">

![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)

</div>

---

## 💼 Featured Projects

<table>
  <tr>
    <td width="50%">
      <h3 align="center">🎬 Movie App</h3>
      <p>A feature-rich movie discovery application with advanced filtering and cart functionality.</p>
      <b>Features:</b>
      <ul>
        <li>Real-time movie search & filtering</li>
        <li>Shopping cart management</li>
        <li>Dark/Light theme toggle</li>
        <li>Fully responsive design</li>
      </ul>
      <b>Tech:</b> React, Tailwind CSS, Context API
    </td>
    <td width="50%">
      <h3 align="center">📚 Virtual Book Shelf</h3>
      <p>Personal book library with authentication and reading progress tracking.</p>
      <b>Features:</b>
      <ul>
        <li>Firebase authentication</li>
        <li>Add/remove books from collection</li>
        <li>Reading progress tracker</li>
        <li>Protected routes & sessions</li>
      </ul>
      <b>Tech:</b> React, Firebase, Tailwind CSS
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3 align="center">🛒 E-commerce UI</h3>
      <p>Modern e-commerce interface with product browsing and checkout flow.</p>
      <b>Features:</b>
      <ul>
        <li>Product filtering & search</li>
        <li>Real-time cart updates</li>
        <li>Modal product preview</li>
        <li>Clean design system</li>
      </ul>
      <b>Tech:</b> React, Tailwind CSS, Components
    </td>
    <td width="50%">
      <h3 align="center">🎓 More Projects Coming</h3>
      <p>I'm constantly building new projects and improving my skills.</p>
      <b>Upcoming:</b>
      <ul>
        <li>Weather App with APIs</li>
        <li>Todo App with Local Storage</li>
        <li>Social Media Clone</li>
        <li>And many more...</li>
      </ul>
    </td>
  </tr>
</table>

---

## 📊 GitHub Statistics

<div align="center">

### 📈 Overall Stats
[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Amdad09&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&bg_color=0d1117&title_color=58A6FF&icon_color=79C0FF&text_color=C9D1D9&include_all_commits=true)](https://github.com/Amdad09)

### 🔥 Contribution Streak
[![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Amdad09&theme=tokyonight&hide_border=true&background=0d1117&ring=58A6FF&fire=FF7E18&stroke=79C0FF)](https://git.io/streak-stats)

### 💻 Most Used Languages
[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Amdad09&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=58A6FF&text_color=C9D1D9)](https://github.com/Amdad09)

</div>

---

## 🎓 Learning Journey

<div align="center">

| Status | Skills |
|--------|--------|
| ✅ **Mastered** | HTML5, CSS3, JavaScript (ES6+), React Hooks, Tailwind CSS, Git & GitHub |
| 🔄 **Currently Learning** | TypeScript, Advanced React Patterns, Redux, REST APIs |
| 📚 **Next Goals** | Node.js, Express, MongoDB, Full Stack Development |

</div>

---

## 🌟 Achievements & Milestones

<div align="center">

- 🎯 Successfully built 5+ production-ready React applications
- 📱 Expert in responsive web design & mobile-first approach
- 🚀 Experienced in Firebase authentication & real-time databases
- 💡 Strong problem-solving skills & debugging capabilities
- 🤝 Collaborative team player with excellent communication

</div>

---

## 💬 Quick Insights

<div align="center">

> **"Code is poetry written for machines, but crafted for humans."**

- 🎨 I believe in clean, readable, and maintainable code
- 🚀 Performance and user experience are my top priorities
- 🌱 Continuous learning is my philosophy
- 🤖 I love automating repetitive tasks

</div>

---

## 📞 Get In Touch

<div align="center">

**Let's connect and collaborate! Reach out to me on:**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/yourprofile)
[![Twitter](https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/yourprofile)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:your.email@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=web&logoColor=white)](https://yourportfolio.com)

</div>

---

<div align="center">
  
### ⭐ Feel free to fork, star, and contribute to my projects!

![Profile Views](https://komarev.com/ghpvc/?username=Amdad09&color=58A6FF&style=for-the-badge)

![Wave Animation](https://raw.githubusercontent.com/BEPb/BEPb/main/assets/Bottom_up.svg)

</div>
